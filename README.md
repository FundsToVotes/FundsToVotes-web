...
We are using [Render](https://render.com) for (free!) Static Site Hosting. When a PR is merged into `main`, the site automatically redeploys. Render has API keys stored in a secret file. Visit the deployed version at [FundsToVotes.info](https://www.fundstovotes.info). As of 5/21/2021, we are no longer actively developing this app, but it will remain available for use at the same link.
